...
export function parseTimeRange(timeRange) {
  ...
  const from = timescale === 'hour' ? moment(to).subtract(quantity, timescale).startOf(timescale).valueOf() : moment(to).subtract(quantity - 1, timescale).startOf(timescale).valueOf();
