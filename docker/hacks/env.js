  ...
  Object.keys(process.env).filter(function(key) {
    ...
    var _val = (function(val) {
      try {
        ...
        if ( toString.call(ret) === '[object Array]' || 
          toString.call(ret) === '[object Array]') {
          return ret;
        }

        //return val if it's not JSON Object or Array
        return val;
      } catch (ignore) {
        return val;
      }
    ...
    if (self.separator) {
      self.set(common.key.apply(common, key.split(self.separator)), _val);
    }
    else {
      self.set(key, _val);
    }
